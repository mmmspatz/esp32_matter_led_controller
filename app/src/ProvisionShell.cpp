...
{
	if (argc != 6) {
		shell_error(sh, "usage: matter_prov set <passcode> <discriminator> <iterations> "
				"<salt_b64> <verifier_b64>");
		return -EINVAL;
	}
	...
	CHIP_ERROR err =
		ZephyrConfig::WriteConfigValue(ZephyrConfig::kConfigKey_SetupPinCode, passcode);
	if (err == CHIP_NO_ERROR) {
		err = ZephyrConfig::WriteConfigValue(ZephyrConfig::kConfigKey_SetupDiscriminator,
						     discriminator);
	}
	if (err == CHIP_NO_ERROR) {
		err = ZephyrConfig::WriteConfigValue(ZephyrConfig::kConfigKey_Spake2pIterationCount,
						     iterations);
	}
	if (err == CHIP_NO_ERROR) {
		err = ZephyrConfig::WriteConfigValueStr(ZephyrConfig::kConfigKey_Spake2pSalt,
							argv[4]);
	}
	if (err == CHIP_NO_ERROR) {
		err = ZephyrConfig::WriteConfigValueStr(ZephyrConfig::kConfigKey_Spake2pVerifier,
							argv[5]);
	}
...
int cmd_show(const struct shell *sh, size_t argc, char **argv)
{
	...
	(void)ZephyrConfig::ReadConfigValue(ZephyrConfig::kConfigKey_SetupDiscriminator,
					    discriminator);
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	sub_matter_prov,
	SHELL_CMD_ARG(set, NULL, "set <passcode> <disc> <iter> <salt_b64> <verifier_b64>", cmd_set,
		      6, 0),
	SHELL_CMD(show, NULL, "show provisioning state", cmd_show),
	SHELL_CMD(clear, NULL, "clear provisioned pairing data", cmd_clear), SHELL_SUBCMD_SET_END);
